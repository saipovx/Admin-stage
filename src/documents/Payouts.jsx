...
export default function Payouts ({ handlePageChange, setcount_documents, page , count_documents}) {
    ...
    const loadData = () => {
      ...
      const loginEndpoint = `/api/payouts/all?limit=25&page=${page}`;
...
{Object.entries(selectedFields).map(([field, isSelected]) => (
    ...
    <div key={field} className={s.static_fitr_flex}>
      ...
      <tr>
      {Object.entries(selectedFields).map(([field, isSelected]) => {
        if (isSelected && translations[field]) {
          return (
            <th key={field} className={s.static__table__thead__titleTwo}>
              {translations[field]}
            </th>
          );
        }
      ...
      {Object.entries(selectedFields).map(([field, isSelected]) => {
        if (isSelected) {
          switch (field) {
            case 'documentNumber':
              return (
                <td key={`${item._id}-documentNumber`} className={s.static__table__tbody__subtitle}>
            ...
            case 'payer':
              return (
                <td key={`${item._id}-payer`} className={s.static__table__tbody__subtitleTwo} id={item.legal_entity_id} onClick={() => navigatePush(item.legal_entity_id)}>
            ...
            case 'status':
              return (
                <td key={`${item._id}-status`} className={s.static__table__tbody__subtitle}>
