...
export default function VenuesAll ({ handlePageChange, setcount_documents, page , count_documents }) {
  ...
  const loadData = () => {
    ...
    const loginEndpoint = `/api/moderation/sites/manager?limit=20&page=${page}`;
          ...
          {VenuesDataType.map((item) => (
            ...
            <tr style={{ borderBottom: '1px solid #EBEDF5' }} className={s.static__table__tbody__tr} key={item.id}>
              ...
              <td className={s.static__table__tbody__subtitle} >
                ...
                {item.campaign && item.campaign.segment} 

              </td>

              <td className={s.static__table__tbody__subtitle} >
                
                {item.campaign && item.campaign.task} 
