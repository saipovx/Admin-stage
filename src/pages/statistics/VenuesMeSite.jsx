...
export default function VenuesMeSite ({ handlePageChange, setcount_documents, page , count_documents }) {
  ...
  const loadData = () => {
    ...
    const loginEndpoint = `/api/moderation/sites/manager?limit=25&page=${page}${searchQuery ? `&search=${searchQuery}` : ''}${selectedOptionTwo ? `&is_moderated=${selectedOptionTwo}` : ''}${status ? `&status=${status}` : ''}&type_platform=site`;
